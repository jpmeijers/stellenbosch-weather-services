...
try:
    ...
    # read in file
#    f = open(dataFile)
    dataFile = dataFile+str(int((delta.days*24*60) + (delta.seconds/60) + 1 ))
    pprint(dataFile)
...
#    if 0:
        ...
        if (countlines >= 2):
            ...
            date = timeDate[0]
            time_var = timeDate[1]
            sqlString = "SELECT * FROM `"+tableName+"` WHERE `Date` = '"+date+"' AND `Time` = '"+time_var+"'"
            cursor.execute(sqlString)
            
            if (cursor.rowcount==0):
                sqlStatement = sqlBaseStatement + ") VALUES ('"+date+"', '"+time_var+"'"
